...
def list_gtd_files_impl(
    ...
    try:
        ...
        # Convert GTD files to dictionary format
        files_data = []
...
def list_gtd_files(vault_path: str, file_type: str | None = None) -> dict[str, Any]:
    """List all GTD files in the vault with optional filtering by file type.
    ...
    Returns:
        Dictionary with status, files list, summary statistics, and vault path.
        Includes suggestion to run setup_gtd_vault if no GTD structure exists.
    """
