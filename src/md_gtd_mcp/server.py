...
(GTD) workflows using Obsidian markdown files. The server implements both tools for
actions (setup_gtd_vault) and resources for read-only data access following MCP
protocol best practices.

Resource Architecture:
- gtd://{vault_path}/files - List GTD files with metadata
- gtd://{vault_path}/files/{file_type} - List filtered GTD files
- gtd://{vault_path}/file/{file_path} - Read single GTD file
- gtd://{vault_path}/content - Read all GTD files with content
- gtd://{vault_path}/content/{file_type} - Read filtered GTD files with content

Tool Architecture:
- setup_gtd_vault - Create GTD folder structure safely
- hello_world - Test tool for server connectivity
"""
